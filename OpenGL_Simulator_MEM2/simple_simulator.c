...
unsigned int MEMORY2[TAMANHO_MEMORIA2]; // Declaração da memória "secundária" de disco (mesmo tamanho que a regular) - MEM2
...
void le_arquivo(char arquivo[], int memflag); // Alterada para ter parametro 2 como 0 para memória ram, e 1 para memória disco
...
int main(int argc, char *argv[])
{
	...
	int HDD_OUT=0; // Para memória de disco - MEM2
    ...
    if (argc > 1) le_arquivo(argv[1], 0);
    else le_arquivo("cpuram.mif", 0);
		if (argc == 3) le_arquivo(argv[2], 1);
		else le_arquivo("cpuhdd.mif", 1);
...
loop:
	...
	if (RW2 == 1) MEMORY2[M1] = M5; // Escrita da memória de disco - MEM2
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case LOAD2: // Load para memoria de disco - MEM2
					// MAR = MEMORY2[PC];
					// PC++;
					selM1 = sPC;
					RW2 = 0;
        ...
        case LOADINDEX2: // Loadi para memoria de disco - MEM2
          // reg[rx] = MEMORY2[reg[ry]];
          selM4 = ry;
          selM1 = sM4;
          RW2 = 0;
          selM2 = sHDD_OUT;
          ...
          break;
				...
				case STORE2: // Store para memoria de disco - MEM2
					//MAR = MEMORY2[PC];
					//PC++;
					selM1 = sPC;
					RW2 = 0;
        ...
        case STOREINDEX2: // Storei para memoria de disco - MEM2
          //mem2[reg[rx]] = reg[ry];
          selM4 = rx;
          selM1 = sM4;
          RW2 = 1;
	...
	if (RW2 == 0) HDD_OUT = MEMORY2[M1]; // Leitura de memória de disco - MEM2
	...
	else if (selM2 == sHDD_OUT) M2 = HDD_OUT; // For storage memory - MEM2
...
void le_arquivo(char arquivo[], int memflag){
    printf("%s\n", arquivo);
	...
	// Seta MEM e TAM para a memória ram caso memflag==0, e para a disco se memflag==1
	if(memflag){
		MEM=MEMORY2;
		TAM=TAMANHO_MEMORIA2;
	}
	else{
		MEM=MEMORY;
		TAM=TAMANHO_MEMORIA;
